# Prompt: Agent 3 — Improvement Plan

> Copy everything below the line into any LLM (ChatGPT, Claude, Gemini, etc.) to run this agent. This is the plain-prompt equivalent of the `/agent-3-improvement-plan` skill in this repo.
>
> **Note for LLMs with file-write access:** If you have tools to write to the local filesystem (e.g. Gemini Code Assist, Claude Code, Cursor, or similar agentic IDEs), you may save your final improvement plan to a markdown file in the current directory so the user can review and accept it. Otherwise, present the output inline in the chat.
